...
def sync_folder(od_client, gd_service, od_folder_id, gd_parent_id, path_prefix="", executor=None, futures=None, creds=None, is_new_folder=False):
    ...
    # Bolt optimization: Skip listing if we just created this folder (it's empty)
    try:
        if is_new_folder:
            gd_folder_contents = {}
        else:
    ...
    for item in items:
        ...
        if item_type == 'folder':
            ...
            try:
                ...
                # Recurse
                sync_folder(od_client, gd_service, item_id, gd_folder_id, current_path, executor, futures, creds, is_new_folder=is_child_new)
            except Exception as e:
